...
{
    public function get_empleados($estadoFilter = null)
    {
        if ($estadoFilter !== null) {
            $this->db->where("estado", $estadoFilter);
        }
    ...
    {
        $cargo = $this->db->get_where("cargo", ["id" => $cargo_id])->row();
    ...
    {
        $this->db->select(
            "empleado.*, sucursal.nombre AS nombre_sucursal, cargo.nombre AS nombre_cargo"
        );
